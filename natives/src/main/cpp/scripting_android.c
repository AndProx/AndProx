...
#include <lualib.h>
#include <lauxlib.h>
// ANDROID HACK: proxmark3.h -> fakemain.h
#include "fakemain.h"
...
// ANDROID HACK: This should be functional
static int l_PrintAndLog(lua_State *L){
    PrintAndLog((char *) luaL_checkstring(L, 1));
    return 0;
}
...
int set_pm3_libraries(lua_State *L)
{
    ...
    // ANDROID-SPECIFIC HACK: Break stdin
    // At the moment, we don't have a good way to plumb this. Without plumbing or breakage in place
    // the library hangs and uses 100% CPU.
    //
    // TODO: implement something like http://lua-users.org/lists/lua-l/2007-04/msg00477.html
    lua_getfield(L, LUA_REGISTRYINDEX, "_IO_input");
    luaL_Stream *p = (luaL_Stream*)lua_touserdata(L, -1);
    p->closef = NULL;

    // END ANDROID-SPECIFIC HACK
    return 1;
}
